/**
 * Nedflix Desktop - Live TV (IPTV) Module
 */
...
async function openLiveTv() {
    ...
    if (liveTvView) liveTvView.style.display = 'flex';
...
function renderLiveTvChannels(channels) {
    ...
    channels.forEach((channel, index) => {
        ...
        html += `
            <div class="livetv-channel-item"
                 data-url="${escapeHtml(channel.url)}"
                 data-id="${escapeHtml(channel.tvgId || channel.id)}"
                 data-name="${escapeHtml(channel.name)}"
                 data-logo="${escapeHtml(channel.tvgLogo || '')}">
