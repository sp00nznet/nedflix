...
static void state_network(void)
{
    ...
    if (network_init() == 0) {
        ...
        timeout++;
        if (timeout > 180) {
            app_set_error("No network.\nPS1 requires serial link\nor Yaroze Net adapter.");
            timeout = 0;
        }
    }
}
...
static void state_menu(void)
{
    ...
    ui_draw_text(20, 200, "2MB RAM - Audio streaming only", 102, 102, 102);
    ...
    if (input_pressed(BTN_CIRCLE)) g_app.running = false;
}
...
static void state_browsing(void)
{
    ...
    if (input_pressed(BTN_CROSS) && g_app.media.count > 0) {
        ...
        if (item->is_directory) {
            ...
            app_set_error("Video requires MDEC\nNot streaming-compatible");
        }
    }
...
static void state_settings(void)
{
    ...
    if (input_pressed(BTN_CIRCLE)) g_app.state = STATE_MENU;
}

static void state_error(void)
{
    ui_draw_error(g_app.error_msg);
    ui_draw_text(80, 200, "X:Retry O:Exit", 204, 204, 204);

    if (input_pressed(BTN_CROSS)) g_app.state = STATE_NETWORK_INIT;
    if (input_pressed(BTN_CIRCLE)) g_app.running = false;
}
