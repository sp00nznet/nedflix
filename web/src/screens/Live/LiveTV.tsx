...
import { getGuide, toggleFavorite } from '../../api/livetv';

const pct = (s: string) => (parseInt(s, 10) || 0) / 100;

// Live TV / EPG guide: favorites row + per-channel program grid.
export function LiveTV() {
  const qc = useQueryClient();
  const q = useQuery({ queryKey: ['guide'], queryFn: getGuide });
  ...
  return (
    <div className="nf-rise" style={{ padding: '48px 40px' }}>
      ...
      <h1 style={{ margin: '0 0 24px', font: `700 40px ${font.ui}`, letterSpacing: '-.02em', color: colors.ink0 }}>Live TV</h1>
          ...
          <div key={c.id} style={{ display: 'flex', gap: 12, alignItems: 'stretch' }}>
            ...
            <div style={{ flex: 1, display: 'flex', gap: 6, minWidth: 0 }}>
              {c.programs.length ? c.programs.map((p, i) => (
                <div key={i} data-focusable tabIndex={0} style={{ flex: p.widthWeight, minWidth: 90, padding: '10px 12px', borderRadius: 10, cursor: 'pointer', background: p.isLive ? 'rgba(251,113,89,.12)' : 'rgba(236,239,247,.045)', border: `1px solid ${p.isLive ? 'var(--ac)' : 'rgba(236,239,247,.08)'}`, overflow: 'hidden' }}>
