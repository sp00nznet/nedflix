...
echo set -e>> "!XISO_SCRIPT!"
echo.>> "!XISO_SCRIPT!"
echo # Install cdrtools ^(provides mkisofs^) if not available>> "!XISO_SCRIPT!"
echo echo "Checking for ISO creation tools...">> "!XISO_SCRIPT!"
echo if ! command -v mkisofs ^&^> /dev/null; then>> "!XISO_SCRIPT!"
echo     echo "Installing cdrtools ^(mkisofs^)...">> "!XISO_SCRIPT!"
echo     pacman -S --noconfirm --needed mingw-w64-x86_64-cdrtools>> "!XISO_SCRIPT!"
echo fi>> "!XISO_SCRIPT!"
echo.>> "!XISO_SCRIPT!"
echo # Also try to get extract-xiso from XboxDev repo>> "!XISO_SCRIPT!"
echo if ! command -v extract-xiso ^&^> /dev/null; then>> "!XISO_SCRIPT!"
echo     echo "Attempting to build extract-xiso...">> "!XISO_SCRIPT!"
echo     XISO_TMP="/tmp/extract-xiso-build">> "!XISO_SCRIPT!"
echo     if [ ! -f "/mingw64/bin/extract-xiso.exe" ]; then>> "!XISO_SCRIPT!"
echo         rm -rf "$XISO_TMP">> "!XISO_SCRIPT!"
echo         git clone --depth 1 https://github.com/XboxDev/extract-xiso.git "$XISO_TMP" 2^>/dev/null ^|^| true>> "!XISO_SCRIPT!"
echo         if [ -d "$XISO_TMP" ]; then>> "!XISO_SCRIPT!"
echo             cd "$XISO_TMP">> "!XISO_SCRIPT!"
echo             mkdir -p build ^&^& cd build>> "!XISO_SCRIPT!"
echo             cmake .. -G "MSYS Makefiles" 2^>/dev/null ^|^| cmake .. 2^>/dev/null ^|^| true>> "!XISO_SCRIPT!"
echo             make 2^>/dev/null ^|^| true>> "!XISO_SCRIPT!"
echo             if [ -f "extract-xiso.exe" ]; then>> "!XISO_SCRIPT!"
echo                 cp extract-xiso.exe /mingw64/bin/>> "!XISO_SCRIPT!"
echo                 echo "extract-xiso built and installed!">> "!XISO_SCRIPT!"
echo             fi>> "!XISO_SCRIPT!"
echo             rm -rf "$XISO_TMP">> "!XISO_SCRIPT!"
echo         fi>> "!XISO_SCRIPT!"
echo     fi>> "!XISO_SCRIPT!"
...
echo.>> "!XISO_SCRIPT!"
echo # Try extract-xiso first ^(creates proper Xbox disc image^)>> "!XISO_SCRIPT!"
echo if command -v extract-xiso ^&^> /dev/null; then>> "!XISO_SCRIPT!"
echo     echo "Creating XISO with extract-xiso...">> "!XISO_SCRIPT!"
echo     extract-xiso -c "!ISO_DIR_MSYS!" "!OUTPUT_MSYS!">> "!XISO_SCRIPT!"
echo elif command -v /mingw64/bin/extract-xiso ^&^> /dev/null; then>> "!XISO_SCRIPT!"
echo     echo "Creating XISO with extract-xiso...">> "!XISO_SCRIPT!"
echo     /mingw64/bin/extract-xiso -c "!ISO_DIR_MSYS!" "!OUTPUT_MSYS!">> "!XISO_SCRIPT!"
echo elif command -v mkisofs ^&^> /dev/null; then>> "!XISO_SCRIPT!"
echo     echo "Creating ISO with mkisofs...">> "!XISO_SCRIPT!"
echo     mkisofs -o "!OUTPUT_MSYS!" -iso-level 4 -R -J "!ISO_DIR_MSYS!">> "!XISO_SCRIPT!"
echo elif command -v /mingw64/bin/mkisofs ^&^> /dev/null; then>> "!XISO_SCRIPT!"
echo     echo "Creating ISO with mkisofs...">> "!XISO_SCRIPT!"
...
echo fi>> "!XISO_SCRIPT!"
echo.>> "!XISO_SCRIPT!"
echo echo "XISO creation completed!">> "!XISO_SCRIPT!"
